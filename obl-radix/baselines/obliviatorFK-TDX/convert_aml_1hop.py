"""Convert IBM AML-Data (W4) CSV files to Obliviator FK input format for 1-hop.

AML variant of convert_banking_1hop.py. The only difference is the account
schema: AML accounts carry (account_id, bank_id) rather than
(account_id, balance, owner_id), so the account payload written to src.txt is a
single column (bank_id) instead of two. The transaction handling is identical —
AML's txn.csv (txn_id, acc_from, acc_to, amount, txn_time, currency,
payment_format, is_laundering) is a superset of the five columns this converter
reads, and the extra columns are ignored by name-based lookup.
...
Usage:
  python3 convert_aml_1hop.py <account.csv> <txn.csv> <out_src.txt> <out_dst.txt>
"""
...
def main():
    ...
    with open(account_file) as f:
        ...
        try:
            i_id, i_bank = (header.index(c) for c in ("account_id", "bank_id"))
        except ValueError as e:
            sys.exit(f"{account_file}: missing column in header {header}: {e}")
        for line in f:
            parts = line.strip().split(',')
            if len(parts) <= max(i_id, i_bank):
                continue
            accounts.append((parts[i_id], parts[i_bank]))
    ...
    # account data: <bank_id>; txn data: <txn_id,acc_to,amount,txn_time>.
    with open(out_src, 'w') as f:
        f.write(f"{num_accounts} {num_txns}\n\n")
        for aid, bank in accounts:
            f.write(f"{aid} {bank}\n")
        f.write("\n")
    ...
    # account data: <bank_id>; txn data: <txn_id,acc_from,amount,txn_time>.
