...
// OpenSSL helper function (local copy)
static int openssl_aes_ctr_encrypt_local(const uint8_t* key, const uint8_t* plaintext,
                                         uint32_t len, const uint8_t* ctr, uint8_t* output) {
    ...
    int outlen;
    if (EVP_EncryptUpdate(ctx, output, &outlen, plaintext, len) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        return -1;
    }
    int final_len;
    EVP_EncryptFinal_ex(ctx, output + outlen, &final_len);
    EVP_CIPHER_CTX_free(ctx);
    return 0;
}
...
static uint8_t get_switch_bit(ShuffleRNG* rng, uint32_t level, uint32_t position) {
    ...
    *(uint32_t*)(ctr + 12) = position;
    ...
    int status = openssl_aes_ctr_encrypt_local(
        aes_key,
