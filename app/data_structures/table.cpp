...
void Table::shuffle_merge_sort(OpEcall op_type) {
    ...
    // Indirect bitonic spike covers the four branchless comparators that
    // dominate sort time on banking_3hop:
    //   JOIN_ATTR, PAIRWISE, END_FIRST, PADDING_LAST.
    // All four are verified branchless (oblivious_sign + integer masks
    ...
    bool indirect_eligible =
        ...
        (op_type == OP_ECALL_COMPARATOR_PADDING_LAST);
