...
bool entries_equal(const Entry& e1, const Entry& e2, bool check_encrypted = false) {
    ...
    for (int i = 0; i < MAX_ATTRIBUTES; i++) {
        if (e1.column_names[i] != e2.column_names[i]) {
            return false;
        }
    }
...
void test_basic_small_values(sgx_enclave_id_t eid) {
    ...
    for (int i = 0; i < MAX_ATTRIBUTES; i++) {
        ...
        entry.column_names[i] = std::string("col_") + std::to_string(i);
    }
...
void test_boundary_values_within_constraints(sgx_enclave_id_t eid) {
    ...
    for (int i = 0; i < MAX_ATTRIBUTES; i++) {
        ...
        entry.column_names[i] = std::string("bound_") + std::to_string(i);
    }
...
void test_values_outside_constraints(sgx_enclave_id_t eid) {
    ...
    entry.column_names[0] = "S1_S_SUPPKEY";
    entry.column_names[1] = "S1_S_NAME";
    entry.column_names[2] = "S1_S_ADDRESS";
    entry.column_names[3] = "S1_S_NATIONKEY";
    entry.column_names[4] = "S1_S_PHONE";
    entry.column_names[5] = "S1_S_ACCTBAL";
    entry.column_names[6] = "S1_S_COMMENT";
...
void test_negative_values(sgx_enclave_id_t eid) {
    ...
    for (int i = 0; i < MAX_ATTRIBUTES; i++) {
        ...
        entry.column_names[i] = std::string("neg_") + std::to_string(i);
    }
...
void test_multiple_cycles(sgx_enclave_id_t eid) {
    ...
    for (int i = 0; i < MAX_ATTRIBUTES; i++) {
        ...
        entry.column_names[i] = std::string("cycle_") + std::to_string(i);
    }
...
void test_attribute_preservation(sgx_enclave_id_t eid) {
    ...
    for (int i = 0; i < MAX_ATTRIBUTES; i++) {
        ...
        entry.column_names[i] = std::string("attr_") + std::to_string(i);
    }
